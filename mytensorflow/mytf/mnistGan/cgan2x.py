...
glist = []
loadFromFile = ConvNet.openEmptyFileR('cgan2g.txt')
...
dlist = []
loadFromFile = ConvNet.openEmptyFileR('cgan2d.txt')
dcva = ConvNet.addlist(dlist,ConvNet.Conv(inDepth = IMAGE_CHANNEL,outDepth = DF*1,filterSize = 1,loadFromFile = loadFromFile))
...
dfca = ConvNet.addlist(dlist,ConvNet.FC(inDepth = V_DIM,outDepth = DF,loadFromFile = loadFromFile))
dfc0 = ConvNet.addlist(dlist,ConvNet.FC(inDepth = DF*2*t4*s4,outDepth = DF,loadFromFile = loadFromFile))
dfc1 = ConvNet.addlist(dlist,ConvNet.FC(inDepth = DF+DF,outDepth = DF+DF,loadFromFile = loadFromFile))
dfc2 = ConvNet.addlist(dlist,ConvNet.FC(inDepth = DF+DF,outDepth = 1,loadFromFile = loadFromFile))
if loadFromFile:loadFromFile.close()
...
def discriminator(inputT, y_label):
    ...
    a = dfca.getLayer(y_label, isRelu=True, fixed = False)
    _ret = dfc0.getLayer(_ret, isRelu=True, fixed = False)
    _ret = tf.concat([_ret, a], 1)
    _ret = dfc1.getLayer(_ret, isRelu=True, fixed = False)
    _ret = dfc2.getLayer(_ret, isRelu=False, fixed = False)
    return _ret
...
#     exit()
    ...
    D_logits  = discriminator(images,y_label)
    samples = generator(testz,test_label)
    D_logits_F = discriminator(G,y_label)
    ...
    gradients = tf.gradients(discriminator(interpolates,y_label), [interpolates])[0]
    ...
    for idx in xrange(0, 50001):
        ...
        for _ in xrange(2):
            ...
            sess.run(d_optim, feed_dict = {z:batch_z,y_label:yLabel, images:loadedimage})

        batch_z = np.random.uniform(-1, 1, size = (BATCH_SIZE, Z_DIM))
        sess.run(g_optim, feed_dict = {z: batch_z,y_label:yLabel})
        ...
        if idx % 500 == 0:
            ...
            def save(idx, gSaver, dSaver):
                print("start save")
                saveToFile = ConvNet.openEmptyFileW("cgan2g"+str(idx)+".txt")
                for item in gSaver:
                    item(saveToFile)
                saveToFile.flush();saveToFile.close()
 
                saveToFile = ConvNet.openEmptyFileW("cgan2d"+str(idx)+".txt")
                for item in dSaver:
                    item(saveToFile)
                saveToFile.flush();saveToFile.close()
                print("end save")
