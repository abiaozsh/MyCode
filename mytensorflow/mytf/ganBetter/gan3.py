...
GF = 16             # Dimension of G filters in first conv layer. default [64]
DF = 16             # Dimension of D filters in first conv layer. default [64]
Z_DIM = 128
IMAGE_CHANNEL = 3
...
print("startload")
glist = []
loadFromFile = ConvNet.openEmptyFileR('gan3g.txt')
...
dlist = []
loadFromFile = ConvNet.openEmptyFileR('gan3d.txt')
    ...
    while True:
        ...
        if idx % 500 == 0:
            ...
            def save(idx, gSaver, dSaver):
                print("start save")
                saveToFile = ConvNet.openEmptyFileW("gan3g"+str(idx)+".txt")
                for item in gSaver:
                    item(saveToFile)
                saveToFile.flush();saveToFile.close()
 
                saveToFile = ConvNet.openEmptyFileW("gan3d"+str(idx)+".txt")
                for item in dSaver:
                    item(saveToFile)
                saveToFile.flush();saveToFile.close()
                print("end save")
