...
#gram_matrix 会摧毁所有空间相关性，只留下style，
...
def main():
    ...
    content_img = read_image('..\\pilatus800.jpg')
    style_img = read_image('..\\StarryNight.jpg')
    ...
    cost_content = (
                build_content_loss(sess.run(net['conv4_2']) ,  net['conv4_2']) +
                build_content_loss(sess.run(net['conv5_2']) ,  net['conv5_2'])
                )
    
    sess.run([net['input'].assign(style_img)])
    
    c1 = build_style_loss(sess.run(net['conv1_1']) ,  net['conv1_1'])#1, 600, 800, 64
    c2 = build_style_loss(sess.run(net['conv2_1']) ,  net['conv2_1'])#(1, 300, 400, 128)
    c3 = build_style_loss(sess.run(net['conv3_1']) ,  net['conv3_1'])#(1, 150, 200, 256)
    c4 = build_style_loss(sess.run(net['conv4_1']) ,  net['conv4_1'])#(1, 75, 100, 512)
    c5 = build_style_loss(sess.run(net['conv5_1']) ,  net['conv5_1'])#(1, 38, 50, 512)
    ...
    cost_total = cost_content * 5 + cost_style
