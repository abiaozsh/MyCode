...
def main():
    ...
    cost_style = ( 0.5*build_style_loss(sess.run(net['conv1_2']) ,  net['conv1_2']) +
                   1.0*build_style_loss(sess.run(net['conv2_2']) ,  net['conv2_2']) +
                   1.5*build_style_loss(sess.run(net['conv3_4']) ,  net['conv3_4']) +
                   3.0*build_style_loss(sess.run(net['conv4_4']) ,  net['conv4_4']) +
                   8.0*build_style_loss(sess.run(net['conv5_4']) ,  net['conv5_4']))
    
    cost_total = cost_style#cost_content + STYLE_STRENGTH * 
    optimizer = tf.train.AdamOptimizer(1)
    
    #取消随机(未随机化的图像，输出更规整，细节上更倾向于原图)
    #去掉一层conv5_2 (没有影响)
    #去掉 cost_content (没有影响)
    #换成高一层的层 _4 （输出的style变了，圈圈没了， 圈圈貌似是在 5_1 里）
