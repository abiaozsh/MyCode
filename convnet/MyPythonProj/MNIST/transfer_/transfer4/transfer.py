...
def main():
    ...
    #这个 cost_content 似乎没什么用，加不加上没区别   第一 cost_content本身非常小，比cost_style小100倍，第二再加上 STYLE_STRENGTH
    ...
    cost_style = ( 1.0*build_style_loss(sess.run(net['conv1_1']) ,  net['conv1_1']) +
                   ...
                   1.0*build_style_loss(sess.run(net['conv3_1']) ,  net['conv3_1']) +
                   1.0*build_style_loss(sess.run(net['conv4_1']) ,  net['conv4_1']) +
                   1.0*build_style_loss(sess.run(net['conv5_1']) ,  net['conv5_1']))
    
    cost_total = cost_content + cost_style#cost_content + STYLE_STRENGTH * 
