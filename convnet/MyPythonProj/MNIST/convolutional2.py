...
def main(_):
  ...
  conv2,xx = ConvNet.ConvLayer(conv1,filterSize = 5,inDepth = 32,outDepth = 64,convStride = 1,poolSize = 2)
    ...
    #pool_shape = conv2.get_shape().as_list()
  reshape = ConvNet.Conv2FC_Reshape(conv2)#tf.reshape(conv2, [pool_shape[0], pool_shape[1] * pool_shape[2] * pool_shape[3]])
  ...
  loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
      labels=train_labels_node, logits=fc2))
    ...
    for begin in xrange(0, size, BATCH_SIZE):
      ...
      if end <= size:
        ...
        batch_predictions = sess.run(
            train_prediction,
            feed_dict={train_data_node: data[-BATCH_SIZE:, ...]})
