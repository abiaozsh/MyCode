...
extern "C" __declspec(dllexport) void FCFWD(int out_depth, int num_inputs, int i_in_act_w, int i_filters_w, int i_bias_w, int i_out_act_w)
{
	...
	for (int i = 0; i < out_depth; i++)
	{
		...
		for (int d = 0; d < num_inputs; d++)
		{
			...
			a += p_input[i_in_act_w + d] * p_input[i_filters_w + i_num_inputs + d]; // for efficiency use Vols directly for now
		}
		//a += bias.w[i];
		a += p_input[i_bias_w + i];
		p_input[i_out_act_w + i] = a;
	}
...
extern "C" __declspec(dllexport) void FCBWD(int out_depth, int num_inputs, int i_in_act_w, int i_filters_w, int i_in_act_dw, int i_out_act_dw, int i_filters_dw, int i_bias_dw)
{
	for (int i = 0; i < num_inputs; i++)
	{
		//in_act.dw[d] = 0;
		p_input[i_in_act_dw + i] = 0;
	}
	...
	for (int i = 0; i < out_depth; i++)
	{
		...
		float chain_grad = p_input[i_out_act_dw + i];
		for (int d = 0; d < num_inputs; d++)
		{
			//in_act.dw[d] += filters.w[i * num_inputs + d] * chain_grad; // grad wrt input data
			p_input[i_in_act_dw + d] += p_input[i_filters_w + i_num_inputs + d] * chain_grad; // grad wrt input data

			//filters.dw[i * num_inputs + d] += in_act.w[d] * chain_grad; // grad wrt params
			p_input[i_filters_dw + i_num_inputs + d] += p_input[i_in_act_w + d] * chain_grad; // grad wrt params
		}
		//this.bias.dw[i] += chain_grad;
		p_input[i_bias_dw + i] += chain_grad;
	}
						...
						for (int fx = 0; fx < sx; fx++)
						{
							...
							if (oy >= 0 && oy < in_act_sy && ox >= 0 && ox < in_act_sx)
							{
								...
								for (int fd = 0; fd < in_depth; fd++)
								{
									...
									a += p_input[i_filters_w + fidx + fd] * p_input[i_in_act_w + Vidx + fd];
		...
		for (int out_x = 0; out_x < out_sx; out_x++)
		{
			...
			for (int out_y = 0; out_y < out_sy; out_y++)
			{
				...
				for (int d = 0; d < out_depth; d++)
				{
					...
					float chain_grad = p_input[i_out_act_dw + ((out_act_sx * out_y) + out_x) * out_depth + d];
					...
					for (int fy = 0; fy < sy; fy++)
					{
						...
						for (int fx = 0; fx < sx; fx++)
						{
							...
							if (oy >= 0 && oy < in_act_sy && ox >= 0 && ox < in_act_sx)
							{
								...
								{
									// avoid function call overhead (x2) for efficiency, compromise modularity :(
									p_input[i_filters_dw + fidx + fd] += p_input[i_in_act_w + Vidx + fd] * chain_grad;
									p_input[i_in_act_dw + Vidx + fd] += p_input[i_filters_w + fidx + fd] * chain_grad;
								}
							}
						}
					}
					p_input[i_bias_dw + d] += chain_grad;
				}
			}
		}
	}
