...
namespace WindowsFormsApplication4
{
    public partial class Form1 : Form
    {
        ...
        private string receive(Socket clientSocket)
        {
            ...
            while (len3 < 4)
            {
                len3 += clientSocket.Receive(receive, len3, 4 - len3, SocketFlags.None);
            }
            int len = (int)receive[0] + ((int)receive[1] << 8) + ((int)receive[2] << 16) + ((int)receive[3] << 24);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dir.Split('\n').Length - 1; i++)
            {
                ...
                //string file = Send(11, (byte)0, false, out part2);//del
                //if (file != filename)
                //{
                //    textBox1.Text = "error";
                //}
