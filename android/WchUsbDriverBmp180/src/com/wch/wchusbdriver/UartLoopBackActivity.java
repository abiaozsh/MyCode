		...
		public void onClick(View v) {
			...
			if (writeText.length() != 0) {
				...
				for (count_int = 0; count_int < NumBytes; count_int++) {
					writeBuffer[count_int] = (byte) writeText.getText().charAt(
							count_int);
				}
			}
			try {
				...
				Toast.makeText(global_context, "WriteData Error",
						Toast.LENGTH_SHORT).show();
				e1.printStackTrace();
			}
		...
		public void handleMessage(Message msg) {
			try {
				...
				if (actualNumBytes != 0x00) {
					Strbuff += String
							.copyValueOf(readBuffer, 0, actualNumBytes);
					actualNumBytes = 0;
					...
					if (idx >= 0) {
						...
						if (datas.length == 3) {
							...
							double p = Double.parseDouble(datas[1]);
							...
							pa=pa - PB;
							aa=aa - AB;
							writeText.setText(ta + "\r\n" + pa + "\r\n" + aa);
						}
					}

				}
			} catch (Exception ex) {
				readText.setText(ex.toString());
			}
		}
	};
