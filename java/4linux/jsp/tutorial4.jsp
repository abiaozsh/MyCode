<html>
<head>
<script src="../file/jquery-1.12.3.js"></script>
<script>
  function printf(val){
    var str = $("#txtResult").val();
    str = str+val+"\r\n";
    $("#txtResult").val(str);
  }
</script>
</head>
<body>
4:表达式<br/>
在第一讲中我们已经学习了一个表达式“1+1”，表达式还有更复杂的形式。<br/>
表达式是指由符号连接的多个数值或变量等，类似数学中的公式。<br/>
写表达式的目的是让计算机来计算表达式的值，因此有一套规则。<br/>
不同的符号有不同的优先级，* / 高于 + - ,因此先计算乘除，再计算加减。<br/>
其他的符号也有相应的优先级，以后会学到。<br/>
表达式中最重要的是括号，括号的优先级高于一切，如果记不住符号之间的优先级的话，可以使用括号。<br/>
计算机会先计算括号中的运算，然后计算括号外的。<br/>
括号也可以嵌套：(4-(1+1))*3。<br/>
程序：<textarea id="txtSrcCode" style="width:700px;height:200px;">
var a = 1;//这种写法相当于var a; 和 a = 3; 的组合。
var b = 2;
var i;
var j;
i = 5;
j = 2;
var k;
k = (i + j) * (a + b);//先计算 i + j 再计算 a + b ，然后将两个结果相乘，积放到k中去。
var l;
l = i + j * a + b;//如果不加括号，先计算j * a 再把结果和 i 和 b 相加，放到l中。
var m;
m = (i - (a + b)) * j;//先计算a + b，再被 i 减，再把结果和 b 相乘，放到m中。
printf("k:" + k);//这里的 + 表示字符串连接，字符串也是表达式的一种。以后会讲到。
printf("l:" + l);
printf("m:" + m);
</textarea><br/>
<input type="button" value="执行" id="buttonExec"></input><br/>
运行结果：<textarea id="txtResult" style="width:300px;height:100px;"></textarea>
<script>
$("#buttonExec").bind("click",function(){
  try{
  eval("\"use strict\";"+$("#txtSrcCode").val());
  }catch(e){
  $("#txtResult").val(e);
  }
});
</script>
</body>
</html>

