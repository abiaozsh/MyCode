...
namespace myd3d
{
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void load()
		{
			...
			{
				p.model = new Model(device.device, "ball.x");
		...
		private PSpot addSpot(double posx, double posy, double posz, double headingx, double headingy, double headingz, List<PSpot> spotList, List<PSpot> traceList, int total, double mass)
		{
			...
			sp.model = new Model(device.device, "ball.x", "earth.bmp");
		...
		private void init1()
		{
...

			{
				...
				for (int i = 0; i < 100; i++)
				{
					...
					addSpot(x + x0, y + y0, z + z0, qx + mx0, 0 + my0, qz + mz0, spotList, traceList, total, 1);
		...
		private void setObject()
		{
			...
			foreach (PSpot p in traceList)
			{
				if (p.trace[tick] != null)
				{
					...
					Vector3 vec = p.trace[tick].pos.getVect();
					vec.Multiply(scale);
					p.model.move(vec);
					...
					for (int i = 0; i < p.lineCount; i++)
					{
						...
						{
							p.line.addNode(p.trace[p.trace.Length - 1].pos.getVect(), Color.FromArgb(0, r, g, b).ToArgb());
						}
						else if (tick - i < 0)
						{
							p.line.addNode(p.trace[0].pos.getVect(), Color.FromArgb(0, r, g, b).ToArgb());
						}
						else
						{
							p.line.addNode(p.trace[tick - i].pos.getVect(), Color.FromArgb(0, r, g, b).ToArgb());
						}
					}
					p.line.Finish();
					device.addObject(p.line);
