...
__inline __m128d LengthD(__m128d vec1,__m128d vec2)
{
...
void merge(int sp,PSpotD* spotSp, int i, PSpotD* allSpot, int* length)
{
...
void CalcGravity(int sp, PSpotD* allSpot, int* length)
{
	__m128d force1 = _mm_set1_pd(0);
	__m128d force2 = _mm_set1_pd(0);
	
	PSpotD* spotSp = &allSpot[sp];
	...
	for(int i = 0;i < sp;i++)
	{
		...
		__m128d r = LengthD(diff1, diff2);

		if (r.m128d_f64[0] < (spotSp->qmass + allSpot[i].qmass)*0.1)
		{
			merge(sp, spotSp, i, allSpot, length);
			return;
		}

		double f = (G * spotSp->mass * allSpot[i].mass) / (r.m128d_f64[0] * r.m128d_f64[0] * r.m128d_f64[0]);
		force1 = _mm_add_pd(force1,_mm_mul_pd(diff1, _mm_set1_pd(f)));
		force2 = _mm_add_sd(force2,_mm_mul_sd(diff2, _mm_set_sd(f)));
	...
	__m128d forcef = LengthD(force1, force2);
...
void ProcessGravityD(PSpotD* spotList,int length)
{
	int len = length;
	while (true)
	{
...
extern "C" __declspec(dllexport) void ProcessGravityDouble(
		double* posx,
		double* posy,
		double* posz,
		double* headingx,
...
{
	PSpotD* spotList = (PSpotD*)_aligned_malloc(sizeof(PSpotD)*length,16);
	...
	ProcessGravityD(spotList, length);
