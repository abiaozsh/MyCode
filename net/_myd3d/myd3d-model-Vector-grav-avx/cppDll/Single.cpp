...
void CalcGravityS(int sp, PSpotS* allSpot,int length)
{
	__m128 force;
	force = _mm_set_ps1(0);
...
void ProcessGravityS(PSpotS* spotList,int length)
{
	while (true)
	{
		float process = 1;
		...
		if (sp != -1)
		{
			CalcGravityS(sp, spotList, length);
		}
		else
		{
			break;
...
extern "C" __declspec(dllexport) void ProcessGravitySingle(
		float* posx,
		float* posy,
		float* posz,
		float* headingx,
	...
	)
{
	...
	PSpotS* spotList = (PSpotS*)_aligned_malloc(sizeof(PSpotS)*length,16);
	...
	ProcessGravityS(spotList, length);
