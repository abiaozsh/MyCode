		...
		private void init()
		{
...

			{
...

				{
					...
					moon = addSpot(y + x0, x + y0, z + z0, qx + mx0, 0 + my0, qz + mz0, spotList, traceList, total, 6.15);
...

				{
					...
					addSpot(y + x0, x + y0, z + z0, qx + mx0, 0 + my0, qz + mz0, spotList, traceList, total, 0.001);

				}


				{
					double ang = 3.14159265 * (60f / 180f);
					double dist = 5;
					...
					sp1 = addSpot(x + x0, y + y0, z + z0, qx + mx0, 0 + my0, qz + mz0, spotList, traceList, total, 0.001);
                ...
                //    addSpot(x + x0, y + y0, z + z0, qx + mx0, 0 + my0, qz + mz0, spotList, traceList, total, 0.001);
			...
			for (int cnt = 0; cnt < total; cnt++)
			{
				...
				if (cnt >= 270 && cnt <= 270 + 2)
				{
					double direction = 5.737;
					double angle = 0;
					double speed = 0.02;
					sp1.heading.X += speed * Math.Sin(direction) * Math.Cos(angle);
					sp1.heading.Y += speed * Math.Sin(angle);
					sp1.heading.Z += speed * Math.Cos(direction) * Math.Cos(angle);
				}
				//if (cnt == 280)
				//{
				//    double direction = 5.737;
...

				{
					...
					foreach (PSpot s in traceList)
					{
						...
						//s.trace[cnt].x = (float)(Math.Sin(-¦Õ2) * r2);
						//s.trace[cnt].y = (float)newy;
						//s.trace[cnt].z = (float)(Math.Cos(-¦Õ2) * r2);
