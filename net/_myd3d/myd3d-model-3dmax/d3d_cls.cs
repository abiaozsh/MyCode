...
namespace myd3d
{
	...
	public class D3DDevice
	{
		...
		PictureBox renderY;
		PictureBox renderZ;
		PictureBox renderCam;
		private float ratioX;
		private float ratioY;
		private float ratioZ;
		private float ratioCam;

		public D3DCamera camX = new D3DCamera();
		public D3DCamera camY = new D3DCamera();
		public D3DCamera camZ = new D3DCamera();
		public D3DCamera cam = new D3DCamera();

		public Device device = null;
		Dictionary<int, D3DObject> objs = new Dictionary<int, D3DObject>();
		SetObject setObject;
		...
		public D3DDevice(Form form, PictureBox renderX, PictureBox renderY, PictureBox renderZ, PictureBox renderCam, SetObject setObject)
		{
			ratioX = (float)renderX.Height / (float)renderX.Width;
			ratioY = (float)renderY.Height / (float)renderY.Width;
			ratioZ = (float)renderZ.Height / (float)renderZ.Width;
			ratioCam = (float)renderCam.Height / (float)renderCam.Width;
		...
		public D3DObject GetObject(int x, int y)
		{
			D3DCamera cam1 = cam.Clone();
			cam1.x = (float)(cam1.x + eyeSize * Math.Sin(cam1.direction - Math.PI * 0.5) * 0.5);
			cam1.z = (float)(cam1.z + eyeSize * Math.Cos(cam1.direction - Math.PI * 0.5) * 0.5);
			cam1.direction += 0.05f;
			int val = RenderDetective(device, cam1, objs, ratio, x, y);
			D3DObject o = objs.ContainsKey(val) ? objs[val] : null;
			return o;
		}

		private static int RenderDetective(Device device, D3DCamera cam, Dictionary<int, D3DObject> objs, float ratio, int x, int y)
		{
			...
			SurfaceDescription sd = origSurface.Description;
			Surface s = device.CreateOffscreenPlainSurface(sd.Width, sd.Height, sd.Format, Pool.SystemMemory);
			device.GetRenderTargetData(origSurface, s);
			Array a = s.LockRectangle(typeof(int), LockFlags.None, sd.Height, sd.Width);
			int[,] aa = (int[,])a;
			int v = aa[y, x];
			s.UnlockRectangle();
			return v & 0xFFFFFF;
	...
	public class Text : D3DObject
	{
		Mesh mesh = null;
		//Æ«²î,¼·Ñ¹   TODO ÑÕÉ« ²ÄÖÊ
		public Text(D3DDevice device, string text, System.Drawing.Font f, float deviation, float extrusion)
			: base(device)
		{
