...
namespace WindowsFormsApplication1
{
	public class ELF
	{
		...
		public static Head ReadHead(BinaryReader br)
		{
			...
			byte[] strtab = null;
			foreach (var shead in head.sheads)
			{
				if (shead.type == SecType.SHT_STRTAB)
				{
					strtab = shead.data;
				}
			}
			...
			foreach (var shead in head.sheads)
			{
				if (shead.type == SecType.SHT_SYMTAB)
				{
					foreach (var sym in shead.sym)
					{
						if (sym.st_name != 0)
						{
							sym.name = "";
							for (int i = (int)sym.st_name; strtab[i] != 0; i++)
							{
								sym.name += (char)strtab[i];
							}
						}
					}
				}
		...
		public static ProgHead ReadProgHead(BinaryReader br)
		{
			...
			phead.code = DASM.decode(phead.data, phead.p_paddr);
			return phead;
		}
		...
		public class Rela
		{
			public uint r_offset;//Elf32_Addr        r_offset;
			public uint r_info;//Elf32_Word        r_info;
			public int r_addend;//Elf32_Sword        r_addend;
		}
		public static Rela ReadRela(BinaryReader br)
		{
			Rela rela = new Rela();
			rela.r_offset = br.ReadUInt32();//Elf32_Word        sh_name;
			rela.r_info = br.ReadUInt32();//Elf32_Word        sh_type;
			rela.r_addend = br.ReadInt32();//Elf32_Word        sh_flags;
			return rela;
		}
		public class Sym
		{
			public uint st_name; // Elf32_Word        st_name;
			public uint st_value;// Elf32_Addr        st_value;
			public uint st_size; // Elf32_Word        st_size;
			public byte st_info; // unsigned char        st_info;
			public byte st_other; // unsigned char        st_other;
			public ushort st_shndx; // Elf32_Half        st_shndx;
			public string name;
			public string ToString()
			{
				return name;
		...
		public static Sym ReadSym(BinaryReader br)
		{
			Sym sym = new Sym();
			sym.st_name = br.ReadUInt32();//Elf32_Word        sh_name;
			sym.st_value = br.ReadUInt32();//Elf32_Word        sh_type;
			sym.st_size = br.ReadUInt32();//Elf32_Word        sh_flags;
			sym.st_info = br.ReadByte();//Elf32_Word        sh_flags;
			sym.st_other = br.ReadByte();//Elf32_Word        sh_flags;
			sym.st_shndx = br.ReadUInt16();//Elf32_Word        sh_flags;
			return sym;
		}
		...
		public static SecHead ReadSecHead(BinaryReader br)
		{
			...
			shead.type = (SecType)shead.sh_type;
			...
			if (shead.type == SecType.SHT_PROGBITS)
			{
				shead.code = DASM.decode(shead.data, shead.sh_addr);
			}
			if (shead.type == SecType.SHT_SYMTAB)
			{
				int cnt = (int)(shead.sh_size / shead.sh_entsize);
				shead.sym = new Sym[cnt];
				for (int i = 0; i < cnt; i++)
				{
					br.BaseStream.Seek(shead.sh_offset + i * shead.sh_entsize, 0);
					shead.sym[i] = ReadSym(br);
				}
			}
			if (shead.type == SecType.SHT_RELA)
			{
				int cnt = (int)(shead.sh_size / shead.sh_entsize);
				shead.rela = new Rela[cnt];
				for (int i = 0; i < cnt; i++)
				{
					br.BaseStream.Seek(shead.sh_offset + i * shead.sh_entsize, 0);
					shead.rela[i] = ReadRela(br);
				}
			}
			return shead;
		}
