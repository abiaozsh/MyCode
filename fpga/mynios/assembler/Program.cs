...
namespace Assembler
{
	class Program : Base
	{
		...
		public static void compile(string filein, string fileout, string fileoutBin, string filetemp, string type)
		{
			...
			lines.Add(Line.match(".section .text"));
			lines.Add(Line.match("_Z7__eof__v:"));
			lines.Add(Line.match("movhi r2, %hiadj(eof)"));
			lines.Add(Line.match("addi r2, r2, %lo(eof)"));
			lines.Add(Line.match("ret"));
			lines.Add(Line.match(".section .data"));
			lines.Add(Line.match("eof:"));
			lines.Add(Line.match(".zero 4"));
			...
			foreach (var line in lines)
			{
				...
				{
					linespass2.Add(Line.match("blt " + line.op2.text + ", " + line.op1.text + ", " + line.op3.text));
				}
				...
				else if ((new Config("stw reg, gprel")).match(line))//stw r2, %gprel(Sd2Card_status_)(gp)
				{
					...
					//movhi	r1, %hiadj(c)
					linespass2.Add(Line.match("orhi r1, r0, %hiadj(" + line.op2.sym + line.op2.getSymAdj() + ")"));
					//addi	r1, r1, %lo(c)
					linespass2.Add(Line.match("addi r1, r1, %lo(" + line.op2.sym + line.op2.getSymAdj() + ")"));
					//ldw	r4, 0(r1)
