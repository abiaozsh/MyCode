...
namespace Assembler
{
    class Program : Base
    {
        ...
        public static void compile(string filein, string fileout, string filetemp)
        {
            ...
            {
                //02001000
                lines.Add(Line.match("movhi sp, %hiadj(33558528)"));
                lines.Add(Line.match("addi sp, sp, %lo(33558528)"));
            ...
            iscode = true;
            {
                foreach (var item in lines)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Data data = new Data();
                            string txt = line.text.Substring(".string \"".Length, line.text.Length - 1 - ".string \"".Length);
                ...
                if (ins.op != null)
                {
                    if (ins.op.sym != null)
                    {
                        ...
                        if (ins.format == 0)
                        {
                            ...
                            {
                                ins.bitins1 = found.pos - ins.pos - 4;
                            ...
                            {
                                ins.bitins1 = procAdj(ins.op, found.pos);
                            }
                            else
                            {
                                throw new Exception();
                        ...
                        else if (ins.format == 2)
                        {
                            ...
                            ins.bitins1 = found.pos >> 2;
                        }
                    }
                    else
                    {
                        ins.bitins1 = procAdj(ins.op, ins.op.ins.Value);
                    }
                }
...

            {
                ...
                foreach (var ins in insList)
                {
                    ...
                    {
                        insbin = (ins.bitregA << (32 - 5)) | (ins.bitregB << (32 - 10)) | ((ins.bitins1 & 0x0000FFFF) << (6)) | (ins.bitcmd);
                    }
                    else if (ins.format == 1)
                    {
                        insbin = (ins.bitregA << (32 - 5)) | (ins.bitregB << (32 - 10)) | (ins.bitregC << (32 - 15)) | (ins.bitcmd << (6 + 5)) | (0x3A);
                    }
                    else if (ins.format == 2)
                    {
                        insbin = (ins.bitins1 << 6) | (ins.bitcmd);
                    }
                    //bw.Write(insbin);
