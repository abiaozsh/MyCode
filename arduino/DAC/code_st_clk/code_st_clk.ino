...
void loop()
{/*
  uint16_t ByteA = Serial.parseInt();
  uint16_t ByteB = Serial.parseInt();
  SendByte(ByteA);
  SendByte(ByteB);
  PORT_STR_ON;
  PORT_STR_OFF;
  int val0 = analogRead(A5);
  Serial.print("ByteA:");
  Serial.print(ByteA);
  Serial.print(",ByteB:");
  Serial.print(ByteB);
...
{
  //bit0 first
  SendByte(pgm_read_byte_near(AnalogTable+(volt<<1)));
  SendByte(pgm_read_byte_near(AnalogTable+(volt<<1)+1));
  PORT_STR_ON;
  PORT_STR_OFF;
}
