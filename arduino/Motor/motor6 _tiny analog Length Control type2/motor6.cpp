...
void waita() {
	if(drA6)
	{
		...
		aread &= 127;
		AnalogInit();
		uint16_t temp = (((uint32_t)TargetRPM)*aread)>>7;
		while(true)
		{
			if(currTick>=temp){
				CmdPWROff;
