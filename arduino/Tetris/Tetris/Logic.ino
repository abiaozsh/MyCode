...
byte GetBlock(byte num,byte idx,byte i,byte j)
{
  ...
  if((j&1)==1)
  {
    ret = (pgm_read_byte_near(_Block+(num<<5)+(idx<<3)+(i<<2)+(j>>1))>>4)&0x0F;
  }
  else
  {
    ret = pgm_read_byte_near(_Block+(num<<5)+(idx<<3)+(i<<2)+(j>>1))&0x0F;
  }
  return ret;
}
...
byte GetBottom(byte num,byte idx,byte j)
{
  ...
  if((j&1)==1)
  {
    ret = (pgm_read_byte_near(_Bottom+(num<<3)+(idx<<1)+(j>>1))>>4)&0x0F;
  }
  else
  {
    ret = pgm_read_byte_near(_Bottom+(num<<3)+(idx<<1)+(j>>1))&0x0F;
  }
  return ret;
}
...
byte GetLeft(byte num,byte idx)
{
  ...
  if((idx&1)==1)
  {
    ret = (pgm_read_byte_near(_Left+(num<<1)+(idx>>1))>>4)&0x0F;
  }
  else
  {
    ret = pgm_read_byte_near(_Left+(num<<1)+(idx>>1))&0x0F;
  }
  return ret;
}
