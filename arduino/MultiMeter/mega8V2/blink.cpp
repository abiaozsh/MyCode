...
#define BIT_SCL _BV(4)
#define BIT_SDA _BV(5)
...
//默认高电平，低电平有效
#define BTN_UP (!(PINC & _BV(0)))
#define BTN_MID  (!(PINC & _BV(1)))
#define BTN_DOWN  (!(PINC & _BV(2)))
...
//#define ADS1115_PGA_MASK 0x7 << ADS1115_PGA_SHIFT
  ...
  while (true) {
    ...
    if(BTN_UP){
      ...
      //±10.24A/20.48A/40.96A
      if(status==9){
        mux = ADS1115_MUX_DIFF_AIN0_AIN3;
        //0.025 OHM 0.256V to*20  max 10.24A
        float orival = val;
        if(currentLv==0){
          val = val * 10.24 * 1.0;//±0.256 V 10.24A
        }else if(currentLv==1){
          val = val * 20.48 * 1.0;//±0.512 V 20.48A
        }else if(currentLv==2){
          val = val * 40.96 * 1.0;//±1.024 V 40.96A
        }
